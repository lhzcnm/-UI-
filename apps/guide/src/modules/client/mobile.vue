...
import Drawer from '../common/components/clientDrawer.vue'
...
import { menusClientMobile } from '@/utils/menu'
...
const { activeId, getMenuNameById, setActive } = useMenu(menusClientMobile)
...
onMounted(async () => {
  const defaultId = 'phoneclientHeader'
  if (defaultId) {
    setActive(defaultId)
