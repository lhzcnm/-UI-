...
import { menusAdminDesktop } from '@/utils/menu'
...
const { activeId, getMenuNameById, setActive } = useMenu(menusAdminDesktop)
...
const { bindScroll, unbindScroll } = useScrollSync(
  ...
  setActive,
  (id: string) => {
    for (const menu of menusAdminDesktop) {
...
onMounted(async () => {
  const defaultId = 'desktopadminHeader'
  if (defaultId) {
    setActive(defaultId)
