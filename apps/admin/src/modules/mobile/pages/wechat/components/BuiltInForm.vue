...
function handleSelect(id: XNativeSelectValue) {
...
<template>
  <form class="space-y-4" @submit.prevent>
    ...
    <div v-show="form.hasKeywords">
      <label class="block mb-1 text-sm text-label">
        关键字。多个关键字，请使用(|)分隔
      </label>
      <XInput
        v-model="form.keywords"
        placeholder="例如：你好|您好|在吗"
      />
    </div>
    <div>
      <label class="block mb-1 text-sm text-label">附加内容1(可选)</label>
      <XTextarea
        v-model="form.content1"
        rows="5" placeholder="请输入附加内容"
      />
    </div>
    <div>
      <label class="block mb-1 text-sm text-label">附加内容2(可选)</label>
      <XTextarea
