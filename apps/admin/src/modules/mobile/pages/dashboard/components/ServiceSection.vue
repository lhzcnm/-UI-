...
async function getStatData(value: string, serviceId: XNativeSelectValue) {
...
<template>
  <section class="border bg-card rounded">
    <div class="flex flex-col p-3 border-b">
      ...
      <div class="my-2">
        ...
        <NativeSelectService
          v-model="serviceId"
          @change="getStatData(selected, $event)"
        />
      </div>

      <div>
        <label class="block text-sm text-label mb-1">时间</label>
        <XSelect
          v-model="selected"
