...
<template>
  <form class="space-y-3" @submit.prevent>
    ...
    <div class="flex space-x-2">
      <div class="flex-1">
        <label class="block text-sm">点数</label>
        <XInputNumber v-model="form.credits" placeholder="点数" />
      </div>
      <div class="flex-1">
        <label class="block text-sm">现有积分</label>
        <XInput v-model="form.credits" placeholder="现有积分" />
      </div>
    </div>
    <div class="flex space-x-2">
      <div class="flex-1">
        <label class="block text-sm">付款状态</label>
      ...
      <div class="flex-1">
        <label class="block text-sm">付款方式</label>
    ...
    <div>
      <label class="block text-sm">交易ID</label>
      <XInput v-model="form.transactionId" placeholder="交易ID" :disabled="!form.isAdd" />
    </div>
    <div>
      <label class="block text-sm">备注</label>
