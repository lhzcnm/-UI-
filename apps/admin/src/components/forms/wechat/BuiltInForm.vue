...
<template>
  <form class="space-y-4" @submit.prevent>
    <div>
      <label for="type" class="block mb-1 text-sm font-medium text-muted-foreground">消息类型</label>
    ...
    <div>
      <label for="content1" class="block mb-1 text-sm font-medium text-muted-foreground">附加内容1(可选)</label>
      <XTextarea id="content1" v-model="form.content1" rows="5" placeholder="请输入附加内容" />
    </div>
    <div>
      <label for="content2" class="block mb-1 text-sm font-medium text-muted-foreground">附加内容2(可选)</label>
