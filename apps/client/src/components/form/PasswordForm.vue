...
import { useCountdown, CAPTCHA_REG, PASSWORD_REG, PHONE_REG, EMAIL_REG } from '@3un/utils'
...
<template>
  <form class="space-y-2" @submit.prevent="submitForm" autocomplete="off">
    ...
    <div class="flex space-x-2">
      ...
      <XButton type="button" :disabled="isRunning" @click="sendCaptcha">
        {{ isRunning ? `${count} 秒后重发` : '获取验证码' }}
      </XButton>
    </div>
    <XInput v-model="form.password" type="password" placeholder="新密码" />
    <XInput v-model="form.confirmPassword" type="password" placeholder="确认新密码" />
