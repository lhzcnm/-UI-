...
function refreshQrcode() {
  ...
  const response = loginApi.getQrcode()
  response.then(({ data }) => {
...
function validQrcode() {
  const response = loginApi.checkQrcode(nonce.value)
...
<template>
  <div class="flex flex-col items-center justify-center space-y-4 h-full">
    ...
    <div class="relative overflow-hidden w-44 h-44 border rounded-lg select-none">
      ...
      <div v-show="expiredMask" class="absolute top-0 bottom-0 left-0 right-0 flex items-center justify-center bg-white/90">
        <a href="javascript:void(0)" class="flex flex-col items-center space-y-2 text-sm text-muted-foreground hover:text-primary" @click="refreshQrcode">
          <Icon icon="lucide:rotate-cw" class="size-4" />
          <span>点击刷新</span>
        </a>
      </div>
    </div>
    <a href="javascript:void(0)" class="flex items-center space-x-2 text-muted-foreground text-sm hover:underline" @click="mode = 'account'">
      返回账号登录
    </a>
  </div>
</template>
