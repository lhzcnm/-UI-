...
<template>
  <div 
    class="p-6 "
    :class="{
      'flex justify-center items-start min-h-screen space-x-6': filteredData.length < 5, 
      'grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6': filteredData.length >= 5
    }">
    <!-- 无数据提示 -->
    <div
    ...
    <div
      ...
      :key="si"
      :class="{'w-1/5':  filteredData.length < 5}"
