...
const batteryInfo = computed(() => {
  ...
  return {
    ...
    label: battery.IsCharging ? localStore.localData['device_Charging'] : (isFull ? localStore.localData['device_Full'] : localStore.localData['device_Charging']),
...
<template>
  <div class="p-4 overflow-hidden bg-card border rounded-lg">
    <div class="flex items-center justify-between mb-4">
      <h3 class="text-lg font-medium">{{ localStore.localData['device_Battery'] }}</h3>
    ...
    <div class="space-y-2">
      <div class="flex items-center justify-between text-sm">
        <span class="text-muted-foreground">{{ localStore.localData['device_Charge'] }}</span>
        <span class="font-medium">{{ batteryInfo.label }}</span>
      </div>

      <div class="flex items-center justify-between text-sm">
        <span class="text-muted-foreground">{{ localStore.localData['device_Health'] }}</span>
        <span class="font-medium">{{ batteryInfo.healthPercentage }}%</span>
      </div>

      <div class="flex items-center justify-between text-sm">
        <span class="text-muted-foreground">{{ localStore.localData['device_DesignCap'] }}</span>
        <span class="font-medium">{{ batteryInfo.designCapacity }} mAh</span>
      </div>

      <div class="flex items-center justify-between text-sm">
        <span class="text-muted-foreground">{{ localStore.localData['device_MaxCap'] }}</span>
        <span class="font-medium">{{ batteryInfo.nominalChargeCapacity }} mAh</span>
      </div>

      <div class="flex items-center justify-between text-sm">
        <span class="text-muted-foreground">{{ localStore.localData['device_Cycles'] }}</span>
        <span class="font-medium">{{ batteryInfo.cycleCount }} 次</span>
      </div>

      <div class="flex items-center justify-between text-sm">
        <span class="text-muted-foreground">{{ localStore.localData['device_Temp'] }}</span>
