...
import ColumnDelete from '@desktop/pages/submit/components/ColumnDelete.vue'
import ColumnFilter from '@desktop/pages/submit/components/ColumnFilter.vue'
...
import { ASYNC_ORDER_STATUS, ASYNC_ORDER_STATUS_MAP_LOCALE, ORDER_STATUS, ORDER_STATUS_MAP, ServiceFieldType } from '@3un/utils'
...
function processDefaultFieldItem(field: ServiceHeader): XTableV2Column<OrderTableView> {
  ...
  switch (field.itemKey) {
    case 'index':
      return renderIndex(field.itemKey, title, field.width ?? 64)
    ...
    case 'status':
      return renderOrderStatus(field.itemKey, title, field.width ?? 58)
